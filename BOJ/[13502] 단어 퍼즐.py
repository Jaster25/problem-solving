...
def dfs(y, x, word):
    if not is_valid_route(trie, word):
        return

    if word not in checked_words:
        checked_words.add(word)
        if is_in_dict(trie, word):
            global count
            count += 1
            answer.add(word)
